...
import platform
from ai_defender.agent import *
from db_work.cyber_db import *
from security.observation import *
...
app.register_blueprint(security_api) # 추가
...
# Streamlit의 연동을 위한 API 

@app.before_request
def before_request_observation():
    start_observation()
...
def api_events():
    ...
    events = get_attack_events()

    # datetime → JSON 변환
    for event in events:
        if event["발생시간"]:
            event["발생시간"] = (
                event["발생시간"].isoformat()
            )
...
@app.route("/api/battles/stats", methods=["GET"])
def api_battle_stats():
    try:
        ...
        print("[BATTLE STATS ERROR]", str(e))
...
@app.route("/api/battles/history", methods=["GET"])
def api_battle_history():
    try:
        history = get_battle_history(limit=100)
        ...
        print("[BATTLE HISTORY ERROR]", str(e))
